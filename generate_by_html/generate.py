...
The User will give you an example of the question, give another one with the same structure and same misconception.
"""
...
# Try-except to handle interruptions
# try:
for i, qc in enumerate(queries_contentx):
    print(i," - ", len(queries_contentx))
    prompt_user = dedent(
        f"""
        Give another example with the same misconception (DONT CHANGE IT OR ADD ANYTHING AFTER IT, MAKE SURE THE LOGIC IS RIGHT). Answer only with the content of the example and nothing else.
    ...
    responses.append({'query': qc, 'response': response_content})

    # Save responses to a CSV file after each successful API call
    responses_df = pd.DataFrame(responses)
    responses_df.to_csv('responses_3.csv', index=False)

# except Exception as e:
#     print(f"An error occurred: {e}")
#     # Save any responses collected so far before exiting
#     if responses:
#         responses_df = pd.DataFrame(responses)
#         responses_df.to_csv('responses_3.csv', index=False)
